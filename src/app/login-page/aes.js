...
System.EventsManager = function(context) {
    ...
    this.Dispose = function() {
        ...
        for (i = me.Items.length - 1; i >= 0; i = i - 1) {
            item = me.Items[i];
            var eventHandler = item.
            Handler;
            if (eventHandler.Type == "System.EventH andler") eventHandler = eventHandler.InvokeNative;
...
System.IO.MemoryStream = function(buffer) {
    this.Type = "System.IO.MemoryStream";
...
System.Convert.ToBase64String = function(b, wrap) {
    ...
    var dLen = cCnt + (wrap ? (cCnt - 1) / 76 << 1 : 0);
...
System._bitConverter = function() {
            ...
            case "number":
                switch (typeCode) {
                    case System.TypeCode.Single:
                        return this.GetBytesFromNumber(value, 32);
                        break;
                    case System.TypeCode.Double:
                        return this.GetBytesFromNumber(value, 64);
                        break;
                    case System.TypeCode.Int16:
                    case System.TypeCode.UInt16:
                        return this.GetBytesFromInt16Le(value);
                        break;
                    case System.TypeCode.Int32:
                    case System.TypeCode.UInt32:
                        return this.GetBytesFromInt32Le(value);
                        break;
                    default:
                        return this.GetBytesFromInt32Le(value);
                        break;
    ...
    function _GetBytesFromIntArray(array, startIndex, count, typeCode, bigEndian) {
        ...
        for (i = startIndex; i < length; i++) {
            for (b = 0; b < sizeBytes; b++) {
                m = (bigEndian) ? i * sizeBytes + sizeBytes - 1 - b : i * sizeBytes + b;
...
System.Security.Cryptography.Rfc2898DeriveBytes = function(password, salt, iterations) {
    ...
    this.GetBytes = function(cb) {
        ...
        var r = Math.floor(cb % this.HmacLength);
        if (r != 0) l++;
        ...
        for (var i = 1; i <= l; i++) {
            ...
            var count = ((i == l) ? r : this.HmacLength);
...
System.Security.Cryptography.ICryptoTransform = function(algorithm, encryption, rgbIV) {
    ...
    function ECB(input, output) {
        var outputBuffer = algo.Encrypt(algo.Key, input, System.Security.Cryptography.CipherMode.ECB);
        System.Buffer.BlockCopy(outputBuffer, 0, output, 0, blockSizeByte);
    }

    function CBC(input, output) {
        for (var i = 0; i < blockSizeByte; i++) temp[i] ^= input[i];
        ECB(temp, output);
...
System.Security.Cryptography.CryptoStream = function(stream, transform, mode) {
    this.Type = "System.Security.Cryptography.CryptoStream";
    var _stream;
    var _transform;
    var _mode;
    var _currentBlock = new Array;
    var _disposed = false;
    var _flushedFinalBlock = false;
    var _partialCount = 0;
    var _endOfStream = false;
    var _waitingBlock = new Array;
    var _waitingCount = 0;
    var _transformedBlock = new Array;
    var _transformedPos = 0;
    var _transformedCount = 0;
    var _workingBlock = new Array;
    var _workingCount = 0;
    this.Write = function(buffer, offset, count) {
        ...
        while (count > 0) {
            if (_partialCount == _transform.InputBlockSize) {
                var len = _transform.TransformBlock(_workingBlock, 0, _partialCount, _currentBlock, 0);
            ...
            if (_transform.CanTransformMultipleBlocks) {
                ...
                if (numBlock > 0) {
                    ...
                    var len = _transform.TransformBlock(buffer, offset, multiSize, multiBlocks, 0);
                ...
                var len = Math.min(_transform.InputBlockSize - _partialCount, count);
...
System.Security.Cryptography.SHA1 = function() {
    ...
    this.HashFinal = function() { return this._EndHash(); }
    ...
    this._initialize = function() {
        ...
        for (var property in base) { if (typeof(this[property]) == "undefined") { this[property] = base[property]; } }
        this.HashSizeValue = 160;
        this.HashSize = 160;
        this._stateSHA1 = new System.Byte(5);
        this._buffer = new System.Byte(0x40);
        this._expandedBuffer = new System.Byte(80);
        this.InitializeState();
    }
...
System.Security.Cryptography.HMACSHA1 = function(key) {
    this.Type = "System.Security.Cryptography.HMACSHA1";
    this.Name = "HMACSHA1";
    this.Algorithm;
    this.Key;
    this.HashSize = 160;
...
System.Security.Cryptography.RijndaelManaged = function() {
    ...
    this.IV;
    this.Key;
    this.Mode = System.Security.Cryptography.CipherMode.CBC;
    ...
    function keyExpansion(key) {
        ...
        for (i = 0, j = 0; j < keylen; j++, i += 4) { k[j] = key[i] | (key[i + 1] << 8) | (key[i + 2] << 16) | (key[i + 3] << 24); }
        ...
        for (j = 0;
            (j < kc) && (r < rounds + 1);) {
            for (;
                (j < kc) && (t < 4); j++, t++) { keySched[r][t] = tk[j]; }
            if (t == 4) {
                r++;
                t = 0;
            }
        ...
        while (r < rounds + 1) {
            ...
            if (kc != 8) { for (j = 1; j < kc; j++) tk[j] ^= tk[j - 1]; } else {
    ...
    this.Test = function() {
        ...
        var ciph = [0xED, 0xFD, 0x61, 0xCA, 0xBC, 0x18, 0xC4, 0xFE];
        var encrypted = this.Encrypt(key, data);
        var decrypted = this.Decrypt(key, encrypted);
        isSuccess = true;
        for (var i = 0; i < data.length; i++) { if (ciph[i] != encrypted[i] || data[i] != decrypted[i]) { isSuccess = false; break; } }
        return isSuccess;
    }
    ...
    this.Encrypt = function(key, input, mode) {
        ...
        for (var b = 0; b < input.length / bpb; b++) {
            ...
            switch (mode) {
                case System.Security.Cryptography.CipherMode.CBC:
                    for (var i = 0; i < bpb; i++) { block[i] ^= this.IV[b * bpb + i]; }
                    break;
                default:
                    break;
            }
    ...
    this.Decrypt = function(key, input, mode) {
        ...
        if (!key || !input) return;
        if (key.length * 8 != this.KeySize) return;
        if (!mode) mode = System.Security.Cryptography.CipherMode.ECB;
        var expandedKey = new prepare_decryption(key);
        for (var b = 0; b < input.length / bpb; b++) {
            var block = input.slice(b * bpb, (b + 1) * bpb);
            var dBlock = AESdecrypt(block, expandedKey);
            if (mode == System.Security.Cryptography.CipherMode.CBC) { for (var i = 0; i < bpb; i++) { dBlock[i] ^= this.IV[b * bpb + i]; } }
            output = output.concat(dBlock);
        }
        return output;
    }
